...
exports.ticketFormSubmissionEmail = async ({
  ...
  const subject = `${!ticketDelivery ? `(${formatDate(ticketDeliveryDate)}) ` : ''}${leadPassenger} just submitted a form on Dummy Ticket 365`;
  ...
  const htmlContent = `
    ...
    <html lang="en">
      <head>
        ...
        <title>${passengers[0].firstName} ${passengers[0].lastName} just submitted a form on Dummy Ticket 365</title>
        <style>
          body {
  ...
  await sendEmail({ email: 'info@dummyticket365.com', name: leadPassenger, subject, htmlContent });
};
